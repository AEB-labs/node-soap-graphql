...
export class NodeSoapCaller implements SoapCaller {
    ...
    async call(input: SoapCallInput): Promise<any> {
        this.debug(
            () =>
                `call operation '${input.operation.name()}' with args '${inspect(
                    input.graphqlArgs,
                    false,
                    5,
                )}'`,
        ...
        ) => void = this.requestFunctionForOperation(input.operation);
    ...
    protected requestFunctionForOperation(
        ...
        return this.soapClient[operation.service().name()][operation.port().name()][
            operation.name()
        ];
    }
